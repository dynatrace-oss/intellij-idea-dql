...
- Updated DQL & DPL engines
  - including support for the `fieldsSnapshot` command
  - fixed the `search` command parameters (setting them as experimental)
- Small UI improvements
  - For an empty DQL query execution, buttons for additional view settings will now be hidden, as they are not
    applicable
    in such a case
...
- Fixed an `Access from Event Dispatch Thread (EDT) is not allowed` error occurring when opening the tenant selector
  dropdown while editing a DQL file.
...
- DQL queries injected into other languages now support query selection contexts. Previously, for injected fragments,
  always the whole query was used, ignoring the cursor context.
