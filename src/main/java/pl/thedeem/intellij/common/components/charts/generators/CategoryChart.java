    ...
    public @Nullable JComponent createSettingsComponent(
        ...
        settingsPanel.add(LabeledComponent.create(
                new SearchableComboBox<>(
                        ...
                        EMPTY_COMBO_OPTION,
                        selected -> {
                            if (!Objects.equals(chartSettings.get(ChartSettings.SELECTED_SERIES), selected)) {
        ...
        settingsPanel.add(LabeledComponent.create(
                new MultipleValuesSelector<>(
                        ...
                        chartSettings.get(ChartSettings.SELECTED_VALUES, new HashSet<>()),
                        (value, selected) -> {
                            Set<String> values = chartSettings.get(ChartSettings.SELECTED_VALUES, new HashSet<>());
                            if (selected) {
                                values.add(value);
                            } else {
                                values.remove(value);
        ...
        settingsPanel.add(LabeledComponent.create(
                new SearchableComboBox<>(
                        columns.keySet(),
                        chartSettings.get(ChartSettings.GROUP_BY_COLUMN),
                        EMPTY_COMBO_OPTION,
                        selected -> {
                            if (!Objects.equals(chartSettings.get(ChartSettings.GROUP_BY_COLUMN), selected)) {
                                chartSettings.set(ChartSettings.GROUP_BY_COLUMN, selected);
                                settingsChangeListener.consume(true);
                            }
                        }),
        ...
        settingsPanel.add(LabeledComponent.create(
                new SearchableComboBox<>(
                        ...
                        null,
                        selected -> {
                            if (!Objects.equals(chartSettings.get(ChartSettings.CHART_ORIENTATION), selected)) {
                                chartSettings.set(ChartSettings.CHART_ORIENTATION, selected);
                                settingsChangeListener.consume(true);
                            }
                        }) {{
