...
public class DQLExecutionService implements DQLManagedService<QueryConfiguration>, UiDataProvider {
    ...
    public void startExecution(@NotNull QueryConfiguration params) {
        ...
        if (tenant == null) {
            updatePanel(new DQLExecutionErrorPanel(DQLBundle.message("services.executeDQL.errors.missingTenant", configuration.tenant())));
            return;
        }
        ...
        requestToken = this.executeApiCall(() -> {
            ...
            updatePanel(new DQLExecuteInProgressPanel(executedResponse));
            return token;
        });
                ...
                () -> {
                    ...
                    this.executeApiCall(() -> {
                        ...
                        if (result.isFinished()) {
                            ...
                            updatePanel(new DQLExecuteInProgressPanel(result));
                        }
                        return result;
                    });
                },
    ...
    private <T> T executeApiCall(@NotNull Callable<T> apiCall) {
        try {
            ...
            updatePanel(new DQLExecutionErrorPanel(e));
        } finally {
            loading.set(false);
            stopping.set(false);
        }
    ...
    private void updatePanel() {
        if (!isRunning()) {
            try {
                switch (displayMode) {
                    case JSON -> {
                        if (this.errored.get() != null) {
                            updatePanel(new DQLExecutionErrorPanel(this.errored.get()));
                        } else {
                    ...
                    case TABLE -> {
                        if (this.errored.get() != null) {
                            ...
                            updatePanel(new DQLTableResultPanel(result, project));
                        }
                    }
                    case USED_QUERY -> {
                        FormattedLanguageText panel = new FormattedLanguageText(true);
                        updatePanel(panel);
                        panel.showResult(configuration.query(), DQLFileType.INSTANCE, project);
                    }
                }
    ...
    private void updatePanel(@NotNull JComponent component) {
        this.resultPanel.removeAll();
        this.resultPanel.add(component, BorderLayout.CENTER);
